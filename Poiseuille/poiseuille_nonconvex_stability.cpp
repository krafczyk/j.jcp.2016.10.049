...
{ 
  ...
  for(i=0;i<=NX;i++)
   ...
   for(j=0;j<=NY;j++)
   {
	   u_exact[j]=4.0*U_max*(1.0-ylabel.assign(0,j)/Ly)*ylabel.assign(0,j)/Ly;  //analytical solution
   }
		...
		for(i=0;i<=NX;i++) 
		{
				...
				for(k=0;k<Q;k++) 
				{
...

					{
						...
						ff.assign(i,j,k) = a1 * ( F(i,j,ne[k])-dt*w[ne[k]]*Force*c*e[ne[k]][0]/cs_2)  + a2*f(i,j,ne[k]);
