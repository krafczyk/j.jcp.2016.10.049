...
plt.figure(6, figsize=(12,4), dpi=320)

## Load Figure 6 nonconvex data
nonconvex_data = []
first = True
with open("Figure6_nonconvex.out", "r") as nonconvex_datafile:
    ...
    for row in csvdata_read:
        if first:
            ...
            if "nan" not in row[3]:
                nonconvex_data.append([float(row[0]), float(row[2]), float(row[3])])
...
for row in nonconvex_data:
    tau = row[1]
    if tau not in nonconvex_data_dict:
        nonconvex_data_dict[tau] = {'h':[], 'e':[]}
    h = row[0]
    e = row[2]
    if h not in nonconvex_data_dict[tau]['h']:
        nonconvex_data_dict[tau]['h'].append(h)
        nonconvex_data_dict[tau]['e'].append(e)

# Sort data
for tau in nonconvex_data_dict:
    hlist = nonconvex_data_dict[tau]['h']
    elist = nonconvex_data_dict[tau]['e']
    ...
    nonconvex_data_dict[tau]['h'] = hsorted
    nonconvex_data_dict[tau]['e'] = esorted

# Load Figure 3 convex data
convex_data = []
first = True
with open("Figure6_convex.out", "r") as convex_datafile:
...
slopeline_range = [min(make_log10(nonconvex_data_dict[2.0]['h'])), max(make_log10(nonconvex_data_dict[2.0]['h']))]
